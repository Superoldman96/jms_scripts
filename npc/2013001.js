function action(mode, type, selection) {
    ...
    if (em == null) {
	cm.sendOk("ｽﾐｵyｫ皋Aｸﾕ｡C");
	cm.dispose();
	return;
    }
    if (!cm.isLeader()) {
	cm.sendOk("ｽﾐ･sｧAｪｺｶ､ｪﾓｸﾚｻ｡ｸﾜ｡I");
	cm.dispose();
	return;
    }
...
//		    cm.warpParty(920010800); //GARDEN.	
		    ...
		    cm.sendOk("ﾁﾂﾁﾂｧAｬ@ｱﾏ､Fｶｮｨ螳R､kｯｫ｡Iｽﾐｸoｻ｡ｸﾜ｡I");
		}
	    } else {
...
/*		} else {
	...
	case 920010500: //sealed
		...
		if (total < 5) {
		    cm.sendOk("ｳoｸﾌｻﾝｭn､ｭｭﾓ､Hｪｺｭｫｶq､~ｯ牘}ﾃ・);
		} else {
		    var num_correct = 0;
		    for (var i = 0; i < 3; i++) {
...
/*			} else {
			...
			if (num_correct > 0) {
			    cm.sendOk("ｦｳ､@ｭﾓｦa､隱ｺｭｫｶqｬOｹ・ｺ");
			} else {
			    cm.sendOk("ｨC､@ｭﾓｦa､隱ｺｭｫｶqｳ｣ｿ・);
			}
		    }
		}
	    } else {
...
/*			} else {
			    cm.sendOk("Please make room!");
    	    		...
    	    		cm.playSound(true, "Party1/Failed");
			if (num_correct >= 3) {
			    cm.sendOk("､@ｭﾓｺbｱ・O･ｿｽTｪｺ｡C");
			} else {
			    cm.sendOk("ｨ箝ﾓｺbｱ・Oｿ~ｪｺ｡C");
			}
		    }
		}
	    } else {
		cm.sendOk("ｬ螻ｫ｢ｰg･ｴ｡I｡I");
	    }
	    break;
	case 920010800:
	    var eim = cm.getEventInstance();
...
//	    cm.warpParty(920010100);
	    break;
	case 920010900:
	    cm.sendNext("ｳoｸﾌｬOｺﾊｺｻ｡CｧA･i･Hｦbｳoｸﾌｧ茯・@ｨﾇｦnｪFｦ陦Aｦｧﾚ､｣ｻ{ｬｰｧﾚｭﾌｦｳ･H､bｳoｸﾌ｡C"); 
	    break;
	case 920011000:
	    cm.sendNext("ｳoｬOｶｺﾁｪｪｺｩﾐｶ｡｡CｧA･i･Hｦbｳoｸﾌｧ茯・@ｨﾇｦnｪFｦ陦Aｦｧﾚ､｣ｻ{ｬｰｧﾚｭﾌｦｳ･H､bｳoｸﾌ｡C"); 
	    break;
    }
    cm.dispose();
}
